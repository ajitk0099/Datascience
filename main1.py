...
def zoom(scale):
    global SET_WIDTH, SET_HEIGHT
    SET_WIDTH = int(SET_WIDTH * scale)
    SET_HEIGHT = int(SET_HEIGHT * scale)
    canvas.config(width=SET_WIDTH, height=SET_HEIGHT)
...
# Tkinter
window = tkinter.Tk()
window.title("Gully Cricket Drs Review System")
...
btn = tkinter.Button(window, text="Zoom", width=50, command=zoom)
btn.pack()
